...
function construirPDF(p) {
    ...
    doc.setFontSize(18);
    doc.text("ELECTRICIDAD", 20, 15);
    doc.setFontSize(9);
    doc.setFont("Helvetica", "normal");
    doc.text("Instalaciones Eléctricas • Mantenimiento • Obras", 20, 21);
    
    doc.setFont("Helvetica", "bold");
    doc.setFontSize(12);
    doc.text("Ian Busto", 190, 12, { align: "right" });
    doc.setFontSize(9);
    doc.setFont("Helvetica", "normal");
    doc.text("Técnico Electricista", 190, 17, { align: "right" });

    // --- DATOS (Con más espacio entre ellos) ---
    ...
    // Izquierda
    doc.setFont("Helvetica", "bold");
    doc.text(`PRESUPUESTO: ${p.numero}`, 25, 43);
    doc.setFont("Helvetica", "normal");
    doc.text(`Fecha: ${formatearFecha(p.fecha)}`, 25, 48);
    doc.text(`Vence: ${formatearFecha(p.vencimiento)}`, 25, 53);

    // Derecha
    doc.setFont("Helvetica", "bold");
    doc.text("CLIENTE:", 115, 43);
    doc.setFont("Helvetica", "normal");
    doc.text(`${p.nombre} ${p.apellido}`, 115, 48);
    doc.text(`${p.direccion || "---"}`, 115, 53);
    doc.text(`Tel: ${p.telefono || "---"}`, 115, 58);

    // --- TABLAS (Más espacio entre ellas) ---
    let yStart = 68;
